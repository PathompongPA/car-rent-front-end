...
export default function JourneyBooking() {
    ...
    return (
        <div id="journeyBooking" className=" journey-booking >> flex flex-col justify-center items-center | p-4 py-8  pb-0 gap-8 w-full h-full | lg:max-w-7xl lg:snap-center pt-24" >
            <h1 className="journey-booking__title >> hidden md:block text-blue-1 font-black text-title-2 | md:text-title-1">{loader?.contents?.stepBooking[i18n.language]?.title}</h1>
            <div className="journey-flex -card >> flex justify-center items-center flex-wrap | gap-4 w-full h-full | lg:gap-8 ">
                {loader?.contents?.stepBooking[i18n.language]?.card?.map((item, index) =>
                    <button
                        ...
                        <h1 className="journey-booking__card-title >> group-hover:md:text-title-3  group-focus:text-title-3 duration-300  flex justify-center items-center | xl:h-full w-full  h-fit p-4  lg:p-0| text-title-3 | md:p-8 lg:p-8 lg:w-full  | lg:text-title-2" index={index}> {item?.subTitle} </h1>
