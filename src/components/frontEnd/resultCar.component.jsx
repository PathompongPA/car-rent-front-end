...
export default function ResultCar({ filter }) {
    ...
    return (
        <div className="result-car >> flex flex-col  gap-4 flex-wrap justify-center  | min-w-[150px]  lg:min-h-[300px] w-full lg:gap-4  p-4 xl:px-0 md:py-4 | bg-linear-to-b from-white to-blue-2/10  | lg:max-w-7xl md:bg-white md:bg-none snap-start ">
            <div className=" grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 justify-center  md:min-h-[200px] gap-2  w-full">
                            ...
                            className="result-car__card --btn >> flex flex-col items-center |
                                ...
                                md:bg-transparent
                                  ...
                                  relative
                                   "
                            key={`${id}+${index}`} >
                            <img className="result-car__img >> aspect-3/2 bg-blue-2/80 object-cover  duration-1000 hover:object-left | w-full rounded-lg | md:w-full brightness-90 group-hover:brightness-100 " src={carThumbnail} alt={`thumbnail-${brand.brandName}-${carName}`} />
